...
class LSGAN:
	...
	def build_graph(self, is_peason_div):
...
def train(input_dir, save_dir,  batch_size = 32, lr = 5e-5, nb_epoch = 200):
	...
	lsgan = LSGAN(batch_size = batch_size, lr = lr)

	d_optim, lsgan_optim, d_loss, lsgan_loss = lsgan.build_graph(True)
	...
	with tf.Session() as sess:
		...
		for epoch in range(0, nb_epoch + 1):
			...
			for i in range(nb_batches):
				...
				# we could apply several d_optim and then lsgan_optim
				
				for j in range(5):

					real_images_batch = imgs[np.random.randint(nb_samples, size = batch_size)]
			
					sess.run(d_optim, feed_dict = {lsgan.real_images: real_images_batch})

				sess.run(lsgan_optim)

			if epoch % 5 == 0 and epoch != 0:

				sampled_imgs = sess.run(lsgan.sample_image())
